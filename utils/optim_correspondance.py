...
def optim_single_pts(*, path: Union[str, Path],
    ...
    with tqdm(total=np.prod(pts.shape), desc='Optimize single points') as pbar:
        for i, j in np.ndindex(pts.shape):
            pts_ = pts.copy().values[None, ...].repeat(len(permutations), 0)
            pts_[..., i, j] += permutations
            pts_list = list(pts_)
...
IDX_OF_FRAME = 25
optim_single_pts(path=Path('rawData'), n_pixels_for_single_points=50, idx_of_frame=IDX_OF_FRAME)
